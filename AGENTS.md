...
当前项目处于 **需求标注 / 交接阶段**。  
对于 GPT 类代理，本项目当前只允许进行以下工作：

- 在现有代码中补充**精确到具体代码位置**的 `TODO` 注释
- 在 `todoList.md` 中补充、更新待办事项
- 明确描述改动点、状态设计、显示规则、交互逻辑和实现边界

当前 **不要直接编写具体功能实现代码**，除非用户明确取消这条限制。

具体要求：

- `TODO` 注释必须尽量写在**实际要改动的代码附近**
- `TODO` 注释应尽可能具体，避免空泛描述
- 不要写泛泛注释，不要只写“这里有问题”“这里需要优化”“这里要和 Datagrip 一样”这类总括式描述
- `TODO` 注释必须尽可能**精确到当前代码行或当前代码块的直接问题**，让后续代理能通过注释所在位置直接理解“为什么这里错了、具体要改哪一层、预期行为是什么”
- 如果一个问题横跨多个位置，必须分别在各自对应的代码行附近写注释，不能只在上层文件或函数入口写一段笼统总说明
- 写注释时优先说明当前这行/这段代码的实际失效原因、错误状态传播链、缺失的交互逻辑或错误的显示规则，而不是只写目标效果
- 能说明“为什么改、改哪里、触发条件、预期行为、边界情况”时，要直接写清楚
- 如果某个需求涉及多个文件，需要分别在对应代码位置落注释，而不是只写一段总说明
- 除更新 `todoList.md` 和补充代码内 `TODO` 外，不要默认提交具体实现
- 在代码中补充注释或 `TODO` 时，必须显式带上 **`GPT` 标识**，方便后续由 `glm` 搜索
- 推荐格式：
  - `// GPT TODO: ...`
  - `// GPT NOTE: ...`
  - `// GPT FIXME: ...`
- 不要只写裸 `TODO`、`NOTE`、`FIXME`，应优先使用带 `GPT` 前缀的注释格式
- 如果是一组连续说明，首行也必须带 `GPT` 前缀，确保可被稳定检索
...
- 非必要不要强制解包
- 生产代码中避免 `try!`
- 单个 Swift 文件原则上不得超过 300 行；如果确实接近上限，应优先拆分 View、ViewModel、子组件、扩展或辅助类型，而不是继续堆叠职责
