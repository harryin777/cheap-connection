...
### 2.B MySQL 当前待修 Bug
- [ ] 表数据分页条左右翻页按钮点击热区过小：当前 plain button 只有一个 11pt 的 chevron 图标，没有额外 padding、最小 frame 或 contentShape，实际命中区域几乎贴着图标本身，导致用户很容易按不到

### 2.C MySQL 已修复 Bug
- [x] 修复连接切换时表列表缓存丢失的问题（合并数据库列表而非覆盖）
- [x] 修复数据库节点双击无法展开的问题
- [x] 修复分页不显示总行数的问题（添加 COUNT(*) 查询）
- [x] 修复表缓存丢失时仍显示"无表"的问题
- [x] 修改提示文字：数据库选择器从左侧改为右侧
